...
local function make_error_util(ctx, err)
  ...
  local msg = "TiktokEngagementBotSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = TiktokEngagementBotError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == TiktokEngagementBotError then
