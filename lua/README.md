# TiktokEngagementBot Lua SDK



The Lua SDK for the TiktokEngagementBot API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Engagement()` — each with the same small set of operations (`create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
-- Create
local created, err = client:Engagement():create({ action = "example", url = "example" })
...
Create an instance: `local engagement = client:Engagement(nil)`
...
```
lua/
├── tiktok-engagement-bot_sdk.lua    -- Main SDK module
...
The main module (`tiktok-engagement-bot_sdk`) exports the SDK constructor
