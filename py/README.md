...
The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Engagement()` — each
carrying a small, uniform set of operations (`create`) instead of raw URL
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/tiktok-engagement-bot-sdk/releases)) or
...
```python
client = TiktokEngagementBotSDK.test(testopts, sdkopts)
```
...
```
py/
├── tiktokengagementbot_sdk.py         -- Main SDK module
...
The main module (`tiktokengagementbot_sdk`) exports the SDK class.
