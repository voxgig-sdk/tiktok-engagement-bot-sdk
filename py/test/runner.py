# TiktokEngagementBot SDK test runner

from __future__ import annotations
import os
import json

from tiktokengagementbot_sdk.utility.voxgig_struct import voxgig_struct as vs


class TiktokEngagementBotTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TiktokEngagementBotTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TiktokEngagementBotTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TiktokEngagementBotTestRunner.getenv("TIKTOK_ENGAGEMENT_BOT_TEST_LIVE")
        override = TiktokEngagementBotTestRunner.getenv("TIKTOK_ENGAGEMENT_BOT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TiktokEngagementBotTestRunner.getenv(key)
        ...
        explain = TiktokEngagementBotTestRunner.getenv("TIKTOK_ENGAGEMENT_BOT_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TiktokEngagementBotTestRunner._test_control is not None:
            return TiktokEngagementBotTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TiktokEngagementBotTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TiktokEngagementBotTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TiktokEngagementBotTestRunner.load_test_control()
...
def load_env_local():
    TiktokEngagementBotTestRunner.load_env_local()


def env_override(m):
    return TiktokEngagementBotTestRunner.env_override(m)


def entity_data(v):
    return TiktokEngagementBotTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TiktokEngagementBotTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TiktokEngagementBotTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TiktokEngagementBotTestRunner.load_test_control()


def live_client_options():
    return TiktokEngagementBotTestRunner.live_client_options()


def live_delay_ms():
    return TiktokEngagementBotTestRunner.live_delay_ms()
