# tiktok-engagement-bot-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TiktokEngagementBot SDK as
two agent tools — `tiktok-engagement-bot_list` and `tiktok-engagement-bot_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tiktok-engagement-bot-mcp)
make build

# 2. Provide credentials via the environment
export TIKTOK_ENGAGEMENT_BOT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user tiktok-engagement-bot \
  -- /absolute/path/to/tiktok-engagement-bot-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./tiktok-engagement-bot-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// tiktok-engagement-bot_list: first page of records
{ "entity": "engagement" }
{ "entity": "engagement", "query": { } }

// tiktok-engagement-bot_load: one record by id
{ "entity": "engagement", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tiktok-engagement-bot-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tiktok-engagement-bot \
     -- "$PWD"/dist/*/tiktok-engagement-bot-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tiktok-engagement-bot_list` and `tiktok-engagement-bot_load` tools now appear
   in new sessions. Ask the agent to *"list engagement using tiktok-engagement-bot"*
   and it calls `tiktok-engagement-bot_list` with `{"entity":"engagement"}`.
...
```sh
export TIKTOK_ENGAGEMENT_BOT_APIKEY=sk_live_xxx            # API key
export TIKTOK_ENGAGEMENT_BOT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `tiktok-engagement-bot_list` tool
...
### Call the `tiktok-engagement-bot_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `tiktok-engagement-bot_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tiktok-engagement-bot_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TIKTOK_ENGAGEMENT_BOT_APIKEY` | API key sent with every request. |
| `TIKTOK_ENGAGEMENT_BOT_BASE` | Optional override of the API base URL. |
...
```sh
./tiktok-engagement-bot-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tiktok-engagement-bot_load","arguments":{"entity":"engagement","query":{"id":1}}}}'
```
