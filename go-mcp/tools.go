...
type Args struct {
	Entity string         `json:"entity" jsonschema:"engagement"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.TiktokEngagementBotSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "tiktok-engagement-bot_list",
		Description: "List records from TiktokEngagementBot. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "tiktok-engagement-bot_load",
		Description: "Load a single record from TiktokEngagementBot. " +
...
func runOp(client *sdk.TiktokEngagementBotSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.TiktokEngagementBotSDK, name string) (sdk.TiktokEngagementBotEntity, error) {
	switch strings.ToLower(name) {
	case "engagement":
		return client.Engagement(nil), nil
