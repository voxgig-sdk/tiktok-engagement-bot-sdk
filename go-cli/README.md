# tiktok-engagement-bot-cli

boru-driven command-line client **and** interactive REPL for the TiktokEngagementBot
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tiktok-engagement-bot-cli)
make build

# 2. See usage (words, entities, env vars)
./tiktok-engagement-bot-cli --help
...
TIKTOK_ENGAGEMENT_BOT_BASE=https://api.example.com ./tiktok-engagement-bot-cli --help

# 6. No arguments -> interactive REPL
./tiktok-engagement-bot-cli
tiktok-engagement-bot> /help
tiktok-engagement-bot> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tiktok-engagement-bot-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/tiktok-engagement-bot-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `tiktok-engagement-bot>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `engagement`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tiktok-engagement-bot-cli`. |
...
(`./tiktok-engagement-bot-cli <expr>`) and interactively (the REPL), and expressions compose the
