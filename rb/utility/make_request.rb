...
module TiktokEngagementBotUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TiktokEngagementBotResponse.new({})
    ...
    if fetch_err
      ...
      response = TiktokEngagementBotResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TiktokEngagementBotResponse.new(fetched)
    else
