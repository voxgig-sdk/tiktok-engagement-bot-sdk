...
class EngagementEntity
  ...
  def make
    opts = @_entopts.dup
    EngagementEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TiktokEngagementBotHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Engagement, Hash] the current Engagement data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TiktokEngagementBotHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Engagement fields)
  def match_get
  ...
  #
  # @param reqdata [EngagementCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Engagement, Hash] the created Engagement; raises TiktokEngagementBotError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = TiktokEngagementBotHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
