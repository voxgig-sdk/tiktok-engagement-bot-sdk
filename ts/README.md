# TiktokEngagementBot TypeScript SDK



The TypeScript SDK for the TiktokEngagementBot API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Engagement()` — each with a small set of operations (`create`)
...
```ts
const client = new TiktokEngagementBotSDK({ apikey: '...' })
...
```ts
new TiktokEngagementBotSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| `tester(testopts?, sdkopts?)` | `TiktokEngagementBotSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `TiktokEngagementBotSDK.test(testopts?, sdkopts?)` | `TiktokEngagementBotSDK` | Create a test-mode client. |
...
| `client` | `client(): TiktokEngagementBotSDK` | Return the parent SDK client. |
...
├── src/
│   ├── TiktokEngagementBotSDK.ts        # Main SDK class
