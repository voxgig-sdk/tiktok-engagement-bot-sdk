...
//     (\`new TiktokEngagementBotSDK(...)\` / \`TiktokEngagementBotSDK.test(...)\` become
//     \`TiktokEngagementBotSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TiktokEngagementBotSDK'
...
const TEST_SEED = {"entity":{"engagement":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TiktokEngagementBotSDK(...)\` and any
// \`TiktokEngagementBotSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TiktokEngagementBotSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TiktokEngagementBotSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
