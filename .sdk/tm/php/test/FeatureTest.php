...
// TiktokEngagementBot SDK feature test
//
...
class FtCtrl extends TiktokEngagementBotControl
{
...
class FtClient
{
    ...
    private TiktokEngagementBotUtility $utility;

    public function __construct(TiktokEngagementBotUtility $utility, array $options)
    {
    ...
    public function get_utility(): TiktokEngagementBotUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TiktokEngagementBotContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TiktokEngagementBotContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TiktokEngagementBotContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TiktokEngagementBotConfig::make_feature($name);
    ...
    private static function buildUrl(TiktokEngagementBotSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TiktokEngagementBotError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TiktokEngagementBotResponse($fetched);
        }
    ...
    private function fail(TiktokEngagementBotContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TiktokEngagementBotContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
