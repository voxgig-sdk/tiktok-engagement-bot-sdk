...
// TiktokEngagementBot SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TiktokEngagementBotTestFeature extends TiktokEngagementBotBaseFeature
{
    ...
    public function init(TiktokEngagementBotContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TiktokEngagementBotContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TiktokEngagementBotParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TiktokEngagementBotContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TiktokEngagementBotContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TiktokEngagementBotParam::call($ctx, $k);
