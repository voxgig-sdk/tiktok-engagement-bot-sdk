...
// TiktokEngagementBot SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TiktokEngagementBotOmni::makeStructRunner(
    ...
    {
        return TiktokEngagementBotOmni::objmodel(TiktokEngagementBotOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return TiktokEngagementBotOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return TiktokEngagementBotOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TiktokEngagementBotOmni::structfix($now, Struct::undef()))
                        !== json_encode(TiktokEngagementBotOmni::structfix($val, Struct::undef()))) {
