...
//    only - a typed TiktokEngagementBotContext there would make every ctx
...
final class TiktokEngagementBotOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TiktokEngagementBotContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TiktokEngagementBotSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TiktokEngagementBotResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TiktokEngagementBotError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TiktokEngagementBotResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TiktokEngagementBotError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TiktokEngagementBotError(is_string($code) ? $code : '', $msg);
    }
