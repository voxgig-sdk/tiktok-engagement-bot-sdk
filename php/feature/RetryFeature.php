...
// TiktokEngagementBot SDK retry feature
...
class TiktokEngagementBotRetryFeature extends TiktokEngagementBotBaseFeature
{
    ...
    public function init(TiktokEngagementBotContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TiktokEngagementBotContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TiktokEngagementBotContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TiktokEngagementBotContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
