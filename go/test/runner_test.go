...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TIKTOKENGAGEMENTBOT_TEST_LIVE") == "TRUE" ||
		os.Getenv("TIKTOKENGAGEMENTBOT_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TIKTOKENGAGEMENTBOT_TEST_EXPLAIN"); explain != "" {
