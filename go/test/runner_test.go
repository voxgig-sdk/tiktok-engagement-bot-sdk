...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TIKTOK_ENGAGEMENT_BOT_TEST_LIVE") == "TRUE" ||
		os.Getenv("TIKTOK_ENGAGEMENT_BOT_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TIKTOK_ENGAGEMENT_BOT_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.TiktokEngagementBotSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TiktokEngagementBotSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TiktokEngagementBotError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TiktokEngagementBotSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TiktokEngagementBotError{Msg: msg, Code: code}
}
