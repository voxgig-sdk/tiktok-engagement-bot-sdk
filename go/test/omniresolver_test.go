...
import (
	...
	omni "github.com/voxgig-sdk/tiktok-engagement-bot-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TiktokEngagementBotError); is {
				out := map[string]any{
					"name":    "TiktokEngagementBotError",
...
func omniCtx(arg any, client *sdk.TiktokEngagementBotSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TiktokEngagementBotSDK); is {
			client = live
